...
class FocusSquare: SCNNode {
    ...
    // Scale factor for the focus square when it is closed, the original size
    static let scaleForClosedSquare: Float = 0.97
    
    // Side length of the focus square segments when it is open (1x1 square)
    ...
    static let fillColor = UIColor(red: 1, green: 0.92, blue: 0.41, alpha: 1)
    
    
    //MARK: - Properties
    
    /// The most recent position of the focus square based on the curent state.
    ...
    /// List if the segments in the focus square.
    ...
    override init() {
        ...
        let correction: Float = FocusSquare.thickness / 2      // correction to align lines perfectly
        
        s1.simdPosition += float3(-(segmentLength / 2 - correction), -(segmentLength - correction), 0)
        s2.simdPosition += float3(segmentLength / 2 - correction, -(segmentLength - correction), 0)
        ...
        s7.simdPosition += float3(-(segmentLength / 2 - correction), segmentLength - correction, 0)
        s8.simdPosition += float3(segmentLength / 2 - correction, segmentLength - correction, 0)
    ...
    Reduce visual size change with distance by scaling up when close and down when far away.
     These adjustments result in a scale of 1.0x for a distance of 0.7 or less
    ...
    func displayNodeHierarchyOnTop(_ isOnTop: Bool) {
        ...
        // Recursively traverse the node's children to update the rendering order depending on the 'isOnTop' parameter.
...
private func flashAnimation(duration: TimeInterval) -> SCNAction {
    ...
    let action = SCNAction.customAction(duration: duration) { (node, elapsedTime) in
        // animate color from HSB to 48/100/100 and back
